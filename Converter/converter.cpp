...
float Converter::calculateS(HSV code)
{
    if (code.getCMax() == 0)
    {
        ...
        return (code.getRange() / code.getCMax()) * 100;
}
...
float Converter::calculateH(RGB code, HSV codeTwo)
{
    ...
    if (abs(cMax - rPrim) < 1)
    {
        float result = 60.0 * fmod(((gPrim - bPrim) / cMax), 6.0);
        if (result < 0)
        {
            resultMultiplied = (360 + result)*100;
            return resultMultiplied / 100.0;
        }
        else
            
            resultMultiplied = 60.0 * fmod(((gPrim - bPrim) / cMax), 6)*100.0;
            return resultMultiplied / 100.0;
    }
}
